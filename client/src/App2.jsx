...
import { BrowserRouter as Router, Routes, Route, Navigate, useNavigate, Link, Outlet, useParams } from 'react-router-dom'; // Import Outlet and useNavigate, useParams
      ...
      <Routes>
        ...
        <Route path="/" element={<AuthenticatedLayout user={user} onLogout={handleLogout} />}>
          ...
          >
             ...
             <Route path="pending-applications" element={<PendingApplications user={user} />} />
             <Route path="application/:applicationId" element={<ApplicationDetails user={user} />} />
...
function DashboardContent({ user }) {
  ...
  const fetchDashboardData = async () => {
   ...
   const fetchOnboardingStatus = async () => {
       if (!user || !user.id) return; // Don't fetch if user is not logged in or has no ID
       ...
       try {
           ...
           if (response.ok) {
               ...
               // Handle errors (e.g., user not found, no application)
               console.error('Error fetching onboarding status:', data.error || response.statusText);
               // Depending on the backend response, you might set status to 'not_started' or show an error
  ...
  useEffect(() => {
    if (user) { // Only fetch if user is authenticated
      fetchDashboardData();
      fetchOnboardingStatus(); // Fetch onboarding status
    }
...
function PendingApplications({ user }) {
    ...
    useEffect(() => {
        const fetchPendingApplications = async () => {
            ...
            try {
                // Placeholder endpoint - you will need to implement this in the backend
                        ...
                        {pendingApps.map(app => (
                            <li key={app.id} className="bg-white p-4 rounded-md shadow flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">Status: {app.status}</p> {/* Should be 'pending' */}
                                </div>
                                <button
...
function ApplicationDetails({ user }) {
    ...
    useEffect(() => {
        const fetchApplicationDetails = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setError(data.error || `Failed to fetch application details for ID ${applicationId}.`);
                }
            } catch (err) {
        ...
        const fetchEmployees = async () => {
             ...
             try {
                 // Placeholder endpoint - you will need to implement this in the backend
                 // This endpoint should return a list of users with role 'user'
                 ...
                 if (response.ok) {
                     setEmployees(data); // Assuming backend returns [{ id: 1, username: 'employee1' }, ...]
                 } else {
                     console.error('Failed to fetch employees:', data.error || response.statusText);
                 }
             } catch (err) {
    ...
    const handleAssignEmployee = async () => {
        if (!assignedEmployee || !applicationId) return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Update local state or refetch details
                if (application) {
                    setApplication({ ...application, assigned_to_employee_id: parseInt(assignedEmployee, 10) });
                }
            } else {
                alert(`Failed to assign employee: ${result.error || response.statusText}`);
            }
        } catch (err) {
     ...
     const handleUpdateStatus = async (newStatus) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(`Failed to update status: ${result.error || response.statusText}`);
            }
        } catch (err) {
    ...
    // Helper function to render file links (placeholder)
    const renderFileLink = (filePath) => {
        ...
        // For now, just display the path or a placeholder
        return <span className="text-indigo-600 cursor-pointer hover:underline" onClick={() => alert(`Simulating download for: ${filePath}`)}>{path.basename(filePath)}</span>;
    };
        ...
        <div className="space-y-6">
            ...
            <div className="border p-6 rounded-md shadow space-y-4 text-sm text-gray-700">
                ...
                <h4 className="text-lg font-medium text-gray-800 border-b pb-2 mt-4">Investment Profile</h4>
                 ...
                 <p><strong>Investments Owned:</strong> {application.investments_owned ? JSON.parse(application.investments_owned).join(', ') : 'None'}</p>
                 ...
                 <h4 className="text-lg font-medium text-gray-800 border-b pb-2 mt-4">Selected Funds</h4>
                 {application.selected_funds && JSON.parse(application.selected_funds).length > 0 ? (
                       <ul className="space-y-2">
                         {JSON.parse(application.selected_funds).map(fund => (
                           <li key={fund.id}>
                             {fund.name}: ${fund.amount}
                           </li>
                         ))}
